...
public final class AdvancedCalculator extends Calculator {
	...
	public double discount(final double saleAmount, final int yearsAsCustomer, final boolean isBusiness) {
		if (saleAmount < MINIMUM_PURCHASE_AMOUNT || yearsAsCustomer < BRONZE_CUSTOMER_SEGMENT) {
			return 0;
		}
	...
	private double discountForLowPurchases(final int yearsAsCustomer, final boolean isBusiness) {
		if (isBusiness) {
	...
	private double discountForHighPurchases(final int yearsAsCustomer, final boolean isBusiness) {
		if (isBusiness) {
